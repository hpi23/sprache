...
impl Display for Diagnostic<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        let notes: String = self
            ...
            .map(|note| {
                format!(
                    "\n {color}Anmerkung:{ansi_reset} {note}",
        ...
        let markers = match (
            ...
            (_, _) => {
                format!(
                    "{marker} ...\n{space}{color}+ {line_count} weitere Zeile{n}{ansi_reset}",
                    marker = raw_marker
                    ...
                    n = match self.span.end.line - self.span.start.line == 1 {
                        true => "",
                        false => "n",
                    },
                )
        ...
        writeln!(
            f,
            " {color}{lvl}{reset_col} im Dokument {path}:{line}:{col}{ansi_reset}\n{line1}\n{line2}\n{marker}{line3}\n\n {color}{msg}{ansi_reset}{notes}",
            color = ansi_col(color + 30, true),
            lvl = self.level,
            reset_col = ansi_col(39, false),
            path = self.span.start.path,
...
impl Display for DiagnosticLevel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Hint => write!(f, "Hinweis"),
            Self::Info => write!(f, "Information"),
            Self::Warning => write!(f, "Warnung"),
