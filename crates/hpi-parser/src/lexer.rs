...
impl<'src> Lex<'src> for Lexer<'src> {
    fn next_token(&mut self) -> Result<'src, Token<'src>> {
        ...
        let kind = match self.curr_char {
            ...
            Some(':') => char_construct!(self, Colon, DivAssign, _, _),
            ...
            Some(char) if char.is_alphabetic() || char == '_' => return Ok(self.make_name()),
            Some(char) => {
                self.next();
...
impl<'src> Lexer<'src> {
    ...
    fn make_string(&mut self) -> Result<'src, Token<'src>> {
        // skip opening quote
        self.next();
    ...
    fn make_name(&mut self) -> Token<'src> {
        ...
        while self.curr_char.map_or(false, |c| {
            c.is_ascii_alphabetic() || c.is_ascii_digit() || c.is_alphabetic() || c == '_'
        }) {
            self.next()
        }
        let kind = match &self.input[start_loc.byte_idx..self.location.byte_idx] {
            ...
            "falls" => TokenKind::Falls,
            "sonst" => TokenKind::Sonst,
            "als" => TokenKind::Als,
            "beantrage" => TokenKind::Beantrage,
            "von" => TokenKind::Von,
            "ergibt" => TokenKind::Ergibt,
            "Zeiger" => TokenKind::Zeiger,
            "auf" => TokenKind::Auf,
...
mod tests {
    ...
    fn single_tokens() {
        let tests = [
            ...
            // Keywords
            ("ja", Ok(TokenKind::Ja.spanned(span!(0..4)))),
            ("nein", Ok(TokenKind::Nein.spanned(span!(0..5)))),
            ("funk", Ok(TokenKind::Funk.spanned(span!(0..2)))),
            ("setze", Ok(TokenKind::Setze.spanned(span!(0..3)))),
            ("ueberweise", Ok(TokenKind::Ueberweise.spanned(span!(0..6)))),
            ("solange", Ok(TokenKind::Solange.spanned(span!(0..5)))),
            ("abbrechen", Ok(TokenKind::Abbrechen.spanned(span!(0..5)))),
            (
                "weitermachen",
                Ok(TokenKind::Weitermachen.spanned(span!(0..8))),
            ),
            ("falls", Ok(TokenKind::Falls.spanned(span!(0..2)))),
            ("oder", Ok(TokenKind::Sonst.spanned(span!(0..4)))),
            ("als", Ok(TokenKind::Als.spanned(span!(0..2)))),
            // Identifiers
